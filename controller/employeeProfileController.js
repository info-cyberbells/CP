...
export const login = async (req, res) => {
  try {
    ...
    // If employee type is not "1", check employeementStatus
    ...
    const fullUrl = req.protocol + "://" + req.get("host"); // Get full URL (e.g., http://localhost:5500)
    const imagePath = employee.image ? `${fullUrl}${employee.image}` : null; // Ensure proper URL formatting
...
export const fetchAllEmployee = async (req, res) => {
  try {
    // Added .sort({ name: 1 }) to sort employees alphabetically by name
    const employees = await Employee.find({ type: { $ne: 1 } })
      .select('-password -__v')
      .sort({ name: 1 }); // Sort by name in ascending order (A-Z)
