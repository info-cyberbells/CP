...
export const uploadSalarySlip = async (req, res) => {
  try {
    salarySlipUpload(req, res, async (err) => {
      ...
      // Try to find existing salary slip
      if (salarySlipId) {
      ...
      if (!salarySlip) {
        ...
        salarySlip = {
          ...
          remarks: remarks || '',
          uploadedAt: new Date(),
          fileUrl: formatPath(req.file.path), // Use fileUrl to match schema
        };
        ...
        const salarySlipUpdate = {
          ...
          uploadedAt: new Date(),
          fileUrl: req.file ? formatPath(req.file.path) : salarySlip.fileUrl, // Use fileUrl
        };
