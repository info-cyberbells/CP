...
export const getAllEmployeesAttendanceAndTasksByDate = async (req, res) => {
  try {
    ...
    // Fetch tasks for the specified date and previous pending tasks
    const tasks = await Task.find({
      $or: [
        ...
        // Previous tasks with "In Progress" or "Paused" status
        {
          createdAt: { $lt: startOfDay },
          status: "In Progress",
        },
      ],
    }).select("employeeId title description status projectName estimatedMinutes estimatedHours createdAt updatedAt");
...
export const getPreviousDayAutoClockOutEmployees = async (req, res) => {
  try {
    ...
    const autoClockOutEmployees = await Attendance.find({
      date: { $gte: previousDay, $lt: nextDay },
      autoClockOut: true,
    }).populate("employeeId", "name email position image gender");
